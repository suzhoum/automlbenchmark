...
def run(dataset, config):
    ...
    if (
        preset_with_refit_full := (set(presets) & {"good_quality", "high_quality"})
    ...
    """
    _include_test_during_fit = config.framework_params.get(
        "_include_test_during_fit", False
    )
    ...
    if "learning_curves" in training_params:
        ...
        if (
            isinstance(lc, dict)
            and "metrics" not in lc
            and problem_type in _curve_metrics
        ):
    ...
    infer = (
        inference_time_classification
        if is_classification
        else inference_time_regression
    )
    inference_times = {}
    ...
    prob_labels = (
        probabilities.columns.values.astype(str).tolist()
    ...
    learning_curves = (
        ...
        if training_params.get("learning_curves", None)
        else None
    )
    ...
    if predictor._trainer.model_best is not None:
        num_models_ensemble = len(
            predictor._trainer.get_minimum_model_set(predictor._trainer.model_best)
        )
    else:
...
def save_artifacts(predictor, leaderboard, learning_curves, config):
    ...
    try:
        ...
        if "learning_curves" in artifacts:
            ...
            save_json.save(
                path=os.path.join(learning_curves_dir, "learning_curves.json"),
                obj=learning_curves,
            )
